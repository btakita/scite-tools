...
function autocomplete(char)
  ...
  if not txt:match('^[^\n]-:%d+:in') then
    ...
    local text = txt:match("undefined.-'")
    print(txt)
  end
end
...
-- If no text is selected, the single-line brace style block is assumed to be
-- on the current line. It is converted into a do..end style block. If text is
-- selected, the entire lines the beginning and end of the selection are on are
-- searched for a 'do' and 'end'. When found, they are replaced by braces.
function toggle_block()
  ...
  if editor:GetSelText() == '' then
    ...
    if s < pos2 then
      ...
      local code = editor:GetSelText()
      code = code:gsub('^do%s*(|.-|)%s*(.-)%s*end$', '{ %1 %2 }')
      editor:ReplaceSel(code)
      if code:match('  }$') then editor:CharLeft() editor:CharLeft() end
    end
  end
  editor:EndUndoAction()
end

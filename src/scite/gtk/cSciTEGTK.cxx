...
protected:
	...
	// added by Mitchell
	virtual char *InputDialog(const char *title, const char *label, const char *defaultText);
...
// added by Mitchell
char *SciTEGTK::InputDialog(const char *title, const char *label, const char *defaultText) {
	GtkAccelGroup *accel_group = gtk_accel_group_new();
	...
	GtkWidget *dLabel = TranslatedLabel(label);
	gtk_table_attach(GTK_TABLE(table), dLabel, 0, 1, 0, 1,
		(GtkAttachOptions) GTK_FILL, (GtkAttachOptions) 0, 5, 5);
	gtk_widget_show(dLabel);

	GtkWidget *dCombo = gtk_combo_new();
	FillComboFromMemory(dCombo, memInputDialog);
	if (strlen(defaultText) != 0)
		gtk_entry_set_text(GTK_ENTRY(GTK_COMBO(dCombo)->entry), defaultText);
	gtk_table_attach(GTK_TABLE(table), dCombo, 1, 2, 0, 1,
		(GtkAttachOptions) (GTK_EXPAND | GTK_FILL), (GtkAttachOptions) 0, 5, 5);
	gtk_entry_set_activates_default(GTK_ENTRY(GTK_COMBO(dCombo)->entry), TRUE);
	gtk_widget_grab_focus(GTK_WIDGET(dCombo));
	gtk_widget_show(dCombo);
	...
	const char *entryText = (ret == GTK_RESPONSE_ACCEPT) ? gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(dCombo)->entry)) : "";
	char *result = g_strdup(entryText);
	memInputDialog.Insert(SString(result));
...
gint SciTEGTK::Key(GdkEventKey *event) {
	...
	int commandID = 0;
	/* for (int i = 0; kmap[i].msg; i++) { // modified by Mitchell
	...
	/* if ((commandID == IDM_NEXTFILE) || (commandID == IDM_PREVFILE)) { // modified by Mitchell
...
void SciTEGTK::CreateMenu() {
	...
	SciTEItemFactoryEntry menuItems[] = {
																				...
																				{"/File/_New", NULL, menuSig, IDM_NEW, 0},
																				{"/File/_Open...", "<control>R", menuSig, IDM_OPEN, 0},
																				{"/File/Open Selecte_d", NULL, menuSig, IDM_OPENSELECTED, 0},
																				{"/File/_Revert", NULL, menuSig, IDM_REVERT, 0},
																				{"/File/_Close", "<control>X", menuSig, IDM_CLOSE, 0},
																				{"/File/_Save", "<control>O", menuSig, IDM_SAVE, 0},
																				{"/File/Save _As...", "<control><shift>O", menuSig, IDM_SAVEAS, 0},
																				{"/File/Save a Co_py...", NULL, menuSig, IDM_SAVEACOPY, 0},
																				...
																				{"/File/_Print", NULL, menuSig, IDM_PRINT, 0},
																				{"/File/sep1", NULL, NULL, 0, "<Separator>"},
																				{"/File/_Load Session...", NULL, menuSig, IDM_LOADSESSION, 0},
																				{"/File/Sa_ve Session...", NULL, menuSig, IDM_SAVESESSION, 0},
																				{"/File/sep2", NULL, menuSig, IDM_MRU_SEP, "<Separator>"},
																				{"/File/File0", NULL, menuSig, fileStackCmdID + 0, 0},
																				{"/File/File1", NULL, menuSig, fileStackCmdID + 1, 0},
																				{"/File/File2", NULL, menuSig, fileStackCmdID + 2, 0},
																				{"/File/File3", NULL, menuSig, fileStackCmdID + 3, 0},
																				{"/File/File4", NULL, menuSig, fileStackCmdID + 4, 0},
																				{"/File/File5", NULL, menuSig, fileStackCmdID + 5, 0},
																				{"/File/File6", NULL, menuSig, fileStackCmdID + 6, 0},
																				{"/File/File7", NULL, menuSig, fileStackCmdID + 7, 0},
																				{"/File/File8", NULL, menuSig, fileStackCmdID + 8, 0},
																				{"/File/File9", NULL, menuSig, fileStackCmdID + 9, 0},
																				{"/File/sep3", NULL, NULL, 0, "<Separator>"},
																				{"/File/E_xit", "<control><shift>Q", menuSig, IDM_QUIT, 0},
																				...
																				{"/Edit/Cu_t", NULL, menuSig, IDM_CUT, 0},
																				{"/Edit/_Copy", "<control><shift>K", menuSig, IDM_COPY, 0},
																				{"/Edit/_Paste", "<control>U", menuSig, IDM_PASTE, 0},
																				{"/Edit/_Delete", "Del", menuSig, IDM_CLEAR, 0},
																				{"/Edit/Duplicat_e", NULL, menuSig, IDM_DUPLICATE, 0},
																				{"/Edit/Upperca_se", NULL, menuSig, IDM_UPRCASE, 0},
																				{"/Edit/_Lowercase", NULL, menuSig, IDM_LWRCASE, 0},
																				{"/Edit/Select _All", NULL, menuSig, IDM_SELECTALL, 0},
																				{"/Edit/sep2", NULL, NULL, 0, "<Separator>"},
																				{"/Edit/Match _Brace", "<control>M", menuSig, IDM_MATCHBRACE, 0},
																				{"/Edit/Select t_o Brace", "<control><shift>M", menuSig, IDM_SELECTTOBRACE, 0},
																				{"/Edit/S_how Calltip", NULL, menuSig, IDM_SHOWCALLTIP, 0},
																				{"/Edit/Complete S_ymbol", "<alt>space", menuSig, IDM_COMPLETE, 0},
																				{"/Edit/Complete _Word", "<control>space", menuSig, IDM_COMPLETEWORD, 0},
																				//{"/Edit/Expand Abbre_v", NULL, menuSig, IDM_ABBREV, 0},
																				//{"/Edit/_Insert Abbreviation", "<(contr)ol>D", menuSig, IDM_INS_ABBREV, 0},
																				{"/Edit/Toggle Blk Co_mment", "<control>Q", menuSig, IDM_BLOCK_COMMENT, 0},
																				{"/Edit/Bo_x Comment", NULL, menuSig, IDM_BOX_COMMENT, 0},
																				{"/Edit/Stream Comme_nt", NULL, menuSig, IDM_STREAM_COMMENT, 0},
																				...
																				{"/Search/_Find...", NULL, menuSig, IDM_FIND, 0},
																				{"/Search/Find _Next", NULL, menuSig, IDM_FINDNEXT, 0},
																				{"/Search/Find Previou_s", NULL, menuSig, IDM_FINDNEXTBACK, 0},
																				{"/Search/F_ind in Files...", NULL, menuSig, IDM_FINDINFILES, 0},
																				{"/Search/R_eplace...", NULL, menuSig, IDM_REPLACE, 0},
#ifdef INCREMENTAL_SEARCH
																				{"/Search/Incrementa_l Search", NULL, menuSig, IDM_INCSEARCH, 0},
#endif
																				{"/Search/sep3", NULL, NULL, 0, "<Separator>"},
																				{"/Search/_Go To...", "<control>L", menuSig, IDM_GOTO, 0},
																				{"/Search/Toggle Bookmar_k", NULL, menuSig, IDM_BOOKMARK_TOGGLE, 0},
																				{"/Search/Next Book_mark", NULL, menuSig, IDM_BOOKMARK_NEXT, 0},
																				{"/Search/Pre_vious Bookmark", NULL, menuSig, IDM_BOOKMARK_PREV, 0},
																				{"/Search/_Clear All Bookmarks", NULL, menuSig, IDM_BOOKMARK_CLEARALL, 0},

																				{"/_View", NULL, NULL, 0, "<Branch>"},
																				{"/View/Toggle _current fold", NULL, menuSig, IDM_EXPAND, 0},
																				{"/View/Toggle _all folds", NULL, menuSig, IDM_TOGGLE_FOLDALL, 0},
																				{"/View/sep1", NULL, NULL, 0, "<Separator>"},
																				{"/View/Full Scree_n", NULL, menuSig, IDM_FULLSCREEN, "<CheckItem>"},
																				{"/View/_Tool Bar", NULL, menuSig, IDM_VIEWTOOLBAR, "<CheckItem>"},
#if GTK_MAJOR_VERSION >= 2
																				{"/View/Tab _Bar", NULL, menuSig, IDM_VIEWTABBAR, "<CheckItem>"},
#endif
																				{"/View/_Status Bar", NULL, menuSig, IDM_VIEWSTATUSBAR, "<CheckItem>"},
																				{"/View/sep2", NULL, NULL, 0, "<Separator>"},
																				{"/View/_Whitespace", NULL, menuSig, IDM_VIEWSPACE, "<CheckItem>"},
																				{"/View/_End of Line", NULL, menuSig, IDM_VIEWEOL, "<CheckItem>"},
																				{"/View/_Indentation Guides", NULL, menuSig, IDM_VIEWGUIDES, "<CheckItem>"},
																				{"/View/_Line Numbers", NULL, menuSig, IDM_LINENUMBERMARGIN, "<CheckItem>"},
																				{"/View/_Margin", NULL, menuSig, IDM_SELMARGIN, "<CheckItem>"},
																				{"/View/_Fold Margin", NULL, menuSig, IDM_FOLDMARGIN, "<CheckItem>"},
																				{"/View/_Output", NULL, menuSig, IDM_TOGGLEOUTPUT, "<CheckItem>"},
																				...
																				{"/Tools/_Go", "<control>G", menuSig, IDM_GO, 0},

																				{"/Tools/_Quickies", NULL, NULL, 0, "<Branch>"},
																				{"/Tools/_Quickies/Tool0", NULL, menuSig, IDM_TOOLS + 0, 0},
																				{"/Tools/_Quickies/Tool1", NULL, menuSig, IDM_TOOLS + 1, 0},
																				{"/Tools/_Quickies/Tool2", NULL, menuSig, IDM_TOOLS + 2, 0},
																				{"/Tools/_Quickies/Tool3", NULL, menuSig, IDM_TOOLS + 3, 0},
																				{"/Tools/_Quickies/Tool4", NULL, menuSig, IDM_TOOLS + 4, 0},
																				{"/Tools/_Quickies/Tool5", NULL, menuSig, IDM_TOOLS + 5, 0},
																				{"/Tools/_Quickies/Tool6", NULL, menuSig, IDM_TOOLS + 6, 0},
																				{"/Tools/_Quickies/Tool7", NULL, menuSig, IDM_TOOLS + 7, 0},
																				{"/Tools/_Quickies/Tool8", NULL, menuSig, IDM_TOOLS + 8, 0},
																				{"/Tools/_Quickies/Tool9", NULL, menuSig, IDM_TOOLS + 9, 0},

																				{"/Tools/Open _Module", NULL, NULL, 0, "<Branch>"},
																				{"/Tools/Open _Module/Tool10", NULL, menuSig, IDM_TOOLS + 10, 0},
																				{"/Tools/Open _Module/Tool11", NULL, menuSig, IDM_TOOLS + 11, 0},
																				{"/Tools/Open _Module/Tool12", NULL, menuSig, IDM_TOOLS + 12, 0},
																				{"/Tools/Open _Module/Tool13", NULL, menuSig, IDM_TOOLS + 13, 0},
																				{"/Tools/Open _Module/Tool14", NULL, menuSig, IDM_TOOLS + 14, 0},
																				{"/Tools/Open _Module/Tool15", NULL, menuSig, IDM_TOOLS + 15, 0},
																				{"/Tools/Open _Module/Tool16", NULL, menuSig, IDM_TOOLS + 16, 0},
																				{"/Tools/Open _Module/Tool17", NULL, menuSig, IDM_TOOLS + 17, 0},
																				{"/Tools/Open _Module/Tool18", NULL, menuSig, IDM_TOOLS + 18, 0},
																				{"/Tools/Open _Module/Tool19", NULL, menuSig, IDM_TOOLS + 19, 0},
																				...
																				{"/Tools/_Stop Executing", "<control><shift>G", menuSig, IDM_STOPEXECUTE, NULL},
																				{"/Tools/sep1", NULL, NULL, 0, "<Separator>"},
																				{"/Tools/_Next Message", NULL, menuSig, IDM_NEXTMSG, 0},
																				{"/Tools/_Previous Message", NULL, menuSig, IDM_PREVMSG, 0},
																				{"/Tools/Clear _Output", NULL, menuSig, IDM_CLEAROUTPUT, 0},
																				{"/Tools/S_witch Pane", NULL, menuSig, IDM_SWITCHPANE, 0},
																		};

	SciTEItemFactoryEntry menuItemsOptions[] = {
							{"/_Options", NULL, NULL, 0, "<Branch>"},
							{"/Options/Vertical _Split", NULL, menuSig, IDM_SPLITVERTICAL, "<CheckItem>"},
							{"/Options/_Wrap", NULL, menuSig, IDM_WRAP, "<CheckItem>"},
							{"/Options/Wrap Out_put", NULL, menuSig, IDM_WRAPOUTPUT, "<CheckItem>"},
							{"/Options/_Read-Only", NULL, menuSig, IDM_READONLY, "<CheckItem>"},
							{"/Options/sep1", NULL, NULL, 0, "<Separator>"},
							{"/Options/EO_L Characters", NULL, 0, 0, "<Branch>"},
							{"/Options/EOL Characters/CR _+ LF", NULL, menuSig, IDM_EOL_CRLF, "<RadioItem>"},
							{"/Options/EOL Characters/_CR", NULL, menuSig, IDM_EOL_CR, "/Options/EOL Characters/CR + LF"},
							{"/Options/EOL Characters/_LF", NULL, menuSig, IDM_EOL_LF, "/Options/EOL Characters/CR + LF"},
							{"/Options/_Convert EOL Characters", NULL, menuSig, IDM_EOL_CONVERT, 0},
							{"/Options/sep2", NULL, NULL, 0, "<Separator>"},
							{"/Options/Inden_t Settings", NULL, menuSig, IDM_TABSIZE, 0},
							...
							{"/Options/Open Lua Startup Scr_ipt", NULL, menuSig, IDM_OPENLUAEXTERNALFILE, 0},
							{"/Options/sep4", NULL, NULL, 0, "<Separator>"},
							{"/Options/_Edit Properties", NULL, 0, 0, "<Branch>"},
					};
	...
	SciTEItemFactoryEntry menuItemsBuffer[] = {
																							...
																							{"/Buffers/_Previous", "<control><alt>P", menuSig, IDM_PREVFILE, 0},
																							{"/Buffers/_Next", "<control><alt>N", menuSig, IDM_NEXTFILE, 0},
																							{"/Buffers/_Close All", "<control><shift>X", menuSig, IDM_CLOSEALL, 0},
																							{"/Buffers/_Save All", NULL, menuSig, IDM_SAVEALL, 0},
	...
	SciTEItemFactoryEntry menuItemsHelp[] = {
																						...
																						{"/Help/_SciTE Help", NULL, menuSig, IDM_HELP_SCITE, 0},
																						{"/Help/_About SciTE", NULL, menuSig, IDM_ABOUT, 0},
																				};
...
void SciTEGTK::CreateUI() {
	...
	GtkAttachOptions opts1 = static_cast<GtkAttachOptions>(GTK_SHRINK);
	GtkAttachOptions opts2 = static_cast<GtkAttachOptions>(
															GTK_EXPAND | GTK_SHRINK | GTK_FILL);
	GtkWidget *label = TranslatedLabel("Search for:");
	gtk_table_attach(GTK_TABLE(table), label, 0, 1, 0, 1, opts1, opts1, 5, 5);
	gtk_widget_show(label);

	IncSearchEntry = gtk_entry_new();
	gtk_table_attach(GTK_TABLE(table), IncSearchEntry, 1, 2, 0, 1, opts2, opts2, 5, 5);
...
void SciTEGTK::FindIncrementSignal(GtkWidget *entry, SciTEGTK *scitew) {
	...
	if (scitew->findWhat != "") {
		...
		if (!scitew->havefound) {
			GdkColor red = { 0, 0xBF00, 0x0000, 0x0000 };
			gtk_widget_modify_text(GTK_WIDGET(scitew->IncSearchEntry), GTK_STATE_NORMAL, &red);
		} else {
			GdkColor white = { 0, 0x0000, 0x0000, 0x0000};
			gtk_widget_modify_text(GTK_WIDGET(scitew->IncSearchEntry), GTK_STATE_NORMAL, &white);
		}
	}
}
...
void SciTEGTK::FindIncrement() {
	GdkColor white = { 0, 0x0000, 0x0000, 0x0000 };
	gtk_widget_modify_text(GTK_WIDGET(IncSearchEntry), GTK_STATE_NORMAL, &white);
